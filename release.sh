...
build_target() {
  ...
  GOOS="$goos" GOARCH="$goarch" CGO_ENABLED=0 \
    go build -ldflags="-s -w" -o "${staging_dir}/${binary_name}" "$MAIN_PKG"
...
sign_checksums() {
  ...
  gpg --local-user "$GPG_ID" --detach-sign --armor "${DIST_DIR}/SHA256SUMS"
  mv "${DIST_DIR}/SHA256SUMS.asc" "${DIST_DIR}/SHA256SUMS.sig"
}

export_public_key() {
  echo "==> Export clé publique GPG"
  gpg --armor --export "$GPG_ID" > "${DIST_DIR}/gpg.key"
}
